...
export default function UnderwriterQueueClient({ user }: { user: UserProps }) {
  ...
  const avgPd = user.tenantId === "lending_club" ? "5.2%" : "6.8%";
  const avgFraud = user.tenantId === "lending_club" ? "26.8%" : "31.4%";
  const reviewedCount = user.tenantId === "lending_club" ? "18" : "12";
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KpiCard label="Reviewed Today" value={reviewedCount} icon="✅" color="green" />
          <KpiCard label="Avg PD Score" value={avgPd} icon="⚠️" color="purple" />
          <KpiCard label="Avg Fraud Prob" value={avgFraud} icon="🔍" color="red" />
        </div>
                ...
                {table.getHeaderGroups().map((hg) => (
                  <tr key={hg.id}>
                    {hg.headers.map((h) => (
                      <th
                        key={h.id}
                        className="px-4 py-3 text-left font-semibold text-gray-600 whitespace-nowrap cursor-pointer hover:bg-gray-100 select-none text-black bg-[#f9fafb]"
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-lg bg-white shadow-2xl overflow-y-auto">
        <div className="px-6 py-5 border-b flex items-center justify-between text-black">
          <div>
        ...
        <div className="p-6 space-y-5 text-black">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-black bg-white"
