...
interface NLGSummaryWidgetProps {
  tenantId: string;
  tenantName: string;
}

function NLGSummaryWidget({ tenantId, tenantName }: NLGSummaryWidgetProps) {
  const [loading, setLoading] = useState(false);
  const [expanded, setExpanded] = useState(false);
  const [period, setPeriod] = useState("last-30d");
  const [audience, setAudience] = useState("cro");

  const getSummaryForTenant = useCallback((tid: string, p: string) => {
    if (tid === "lending_club") {
      return {
        ...MOCK_SUMMARY,
        ...
        key_metrics_table: MOCK_SUMMARY.key_metrics_table.map((m) => {
          if (m.label === "Approval Rate") return { ...m, value: "71.3%" };
          if (m.label === "Manual Review Queue") return { ...m, value: "28" };
          return m;
        }),
        action_items: [
          "Review A/B Experiment exp-001 (PROMOTE recommendation — p = 0.032, Δ approval +3.4 pp)",
          "Clear 28 manual-review applications ahead of month-end reporting",
          "Schedule quarterly model validation committee review before 2025-04-30",
        ],
        period_label: p,
      };
  ...
  const [summary, setSummary] = useState<ExecutiveSummary>(() => getSummaryForTenant(tenantId, "last-30d"));

  const regenerate = useCallback(async () => {
    setLoading(true);
    await new Promise((r) => setTimeout(r, 1200));
    setSummary({
      ...getSummaryForTenant(tenantId, period),
      generated_at: new Date().toISOString(),
    });
    setLoading(false);
  }, [period, audience, tenantId, getSummaryForTenant]);
...
export default function ExecutiveDashboardClient({ user }: { user: UserProps }) {
  ...
  let approvedCount = decisions.filter((d) => d.decision === "APPROVE").length;
  let rejectedCount = decisions.filter((d) => d.decision === "REJECT").length;
  let reviewCount = decisions.filter((d) => d.decision === "MANUAL_REVIEW").length;
  let total = decisions.length;
  let approvalRate = total ? ((approvedCount / total) * 100).toFixed(1) : "—";
  let avgLoan = total
  ...
  if (user.tenantId === "lending_club") {
    approvalRate = "71.3";
    if (total > 0) {
      approvedCount = Math.round(total * 0.713);
      const remaining = total - approvedCount;
      rejectedCount = Math.round(remaining * 0.8);
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <NLGSummaryWidget tenantId={user.tenantId} tenantName={user.tenantName} />
