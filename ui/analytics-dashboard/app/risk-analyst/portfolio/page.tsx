...
export default function PortfolioPage() {
  ...
  const kpiData = [
    { label: "Total Applications", value: decisions?.total.toLocaleString() ?? "–", icon: "📄", color: "blue" as const },
    { label: "Approval Rate", value: formatPct(decisions?.approval_rate ?? 0), icon: "✅", color: "green" as const },
    { label: "Avg PD Score", value: formatPct((decisions?.avg_pd_score ?? 0) * 100), icon: "⚠️", color: "amber" as const },
    { label: "Fraud Flag Rate", value: decisions ? formatPct((decisions.fraud_flag_count / decisions.total) * 100) : "–", icon: "🔍", color: "red" as const },
